...
- Data-driven move timing and stats; seven street-enemy roles, including dedicated Homeless, Security, and Police rosters, plus Couch; global attack-token limits; wave and boss scaling by player count.
...
- The prevailing **rooted whole-cel authored pixel-animation standard** uses complete pose-integrated character drawings registered to fixed ground, hand, gear, and release anchors; runtime selects authored cels and draws only released projectiles or transient VFX separately. Two hundred two canonical clips provide 1,836 rooted authored keys—8–16 meaningful drawings per clip—on a capture-safe 30 Hz presentation timeline while combat remains fixed at 60 Hz. Dave and Shelly use twelve true stride keys at 15 poses per second for a 0.8-second gait; KO adds 64 strict actor-specific poses, while the 13 named Homeless, Security, and Police actors add 936 dedicated poses. Phase advances from applied travel and survives brief stops. Manifest-backed tests reject duplicates, translation filler, foot-line drift, placeholder aliases, runtime costume overlays, or uneven synthetic in-betweens.
