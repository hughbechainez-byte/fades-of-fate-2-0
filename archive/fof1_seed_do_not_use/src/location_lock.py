...
# Ground-up setpiece levels live outside the four Chapter 1 geography locks.
SETPIECE_LEVEL_IDS = (
    "setpiece_underpass_i8",
)
...
@lru_cache(maxsize=4)
def _load_setpiece_routes_cached(path: str) -> tuple[Mapping[str, Any], ...]:
...
def setpiece_routes(*, project_root: str | Path | None = None) -> tuple[Mapping[str, Any], ...]:
    """Load optional ground-up setpiece routes for character/combat test levels."""

    root = Path(project_root) if project_root is not None else Path(_default_manifest_path()).resolve().parent.parent
    path = (root / "data" / "setpiece_routes.json").resolve()
    if not path.is_file():
        return ()
    return _load_setpiece_routes_cached(str(path))


def setpiece_is_level(level_id: str) -> bool:
    return str(level_id).strip() in SETPIECE_LEVEL_IDS
...
def hydrate_gameplay_locations(
    ...
    hydratable_ids = (*LEVEL_IDS, *SETPIECE_LEVEL_IDS)
    for chapter in gameplay.get("campaign", {}).get("chapters", ()):
        for level in chapter.get("levels", ()):
            if not isinstance(level, dict) or str(level.get("id", "")) not in hydratable_ids:
                continue
            ...
            level["landmarks"] = [
                {
                    ...
                    "layer": (
                        ...
                        if str(landmark["id"])
                        in {"freeway_approach", "i8_underpass", "underpass_core"}
                        else "story_prop"
