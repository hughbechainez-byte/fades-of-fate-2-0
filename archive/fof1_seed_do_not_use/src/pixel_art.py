...
def _location_routes_by_theme() -> dict[str, Mapping[str, Any]]:
    ...
    for route in location_lock.setpiece_routes():
        routes[str(route["theme"])] = route
    return routes
...
# Quality floor for living setpieces (birds/mist/dust/paper share this budget).
# Underpass setpiece prefers denser mist when in-game.
_AMBIENT_PARTICLE_LIMIT = 40
...
_CHAPTER_ONE_AMBIENT_EVENTS: dict[str, tuple[dict[str, object], ...]] = {
    ...
    "seven_eleven_underpass": (
        # Far sky life: flock through the portal openings and slow haze sheets.
        {
            "kind": "birds",
            "plane": "far",
            "start": 1400,
        ...
        # Mid: scrub only — no legacy FoF1 ambient vehicle models on this route.
        {
            "kind": "foliage_sway",
            "plane": "mid",
            "start": 1600,
        ...
        # World: restrained practicals + cool mist (no neon candy chips / no traffic).
        {
            "kind": "underpass_lights",
            "plane": "world",
            "start": 1750,
            "end": 3200,
            "anchor": 2320,
    ...
    # Ground-up 2.0 setpiece: denser mist, no legacy vehicles, no procedural
    # foliage blobs (those read as floating artifacts over the authored plate).
    "underpass_setpiece_v1": (
        {
            "kind": "birds",
            "plane": "far",
            "start": 0,
...
def _draw_gameplay_ground_plane(surface: pygame.Surface, cx: float, world_width: int, theme: str) -> None:
    ...
    # Wetter fight floor for underpass setpieces (approved look: reflective asphalt).
    # Soft wash only — no bright floating specular "chips" that read as debris.
    if _theme_key(theme) in {"seven_eleven_underpass", "underpass_setpiece_v1"}:
        wet_top = min(height - 8, 292)
        for y, alpha in (
            (wet_top, 20),
            (wet_top + 10, 26),
            (wet_top + 22, 32),
            ...
            pygame.draw.rect(overlay, (18, 26, 34, alpha), (0, y, width, max(1, height - y)))
        # Broad low-alpha sheen bands (not discrete floating dots).
        for offset, alpha in ((0, 14), (18, 10), (36, 8)):
            y = min(height - 4, 312 + offset)
            pygame.draw.rect(overlay, (190, 145, 100, alpha), (0, y, width, 2))
...
def _draw_ambient_particle(
    ...
    if kind == "paper":
        ...
        # Cool gray haze — not electric cyan.
        mist = (148, 156, 160, 18 + size * 6)
        pygame.draw.ellipse(surface, mist, (x, y, 16 + size * 7, 4 + size * 2))
        pygame.draw.rect(surface, (168, 172, 170, 14 + size * 4), (x + 5, y + 2, 12 + size * 5, 2))
...
def _draw_ambient_birds(
    ...
    for index in range(count):
        ...
        wing = 2 + ((motion_tick // 4 + index) % 2) * 3
        # Slightly brighter silhouette so portal-sky flocks read at 640x360.
        color = (28, 24, 34, 210)
        pygame.draw.line(surface, color, (x - 6, y + wing), (x, y), 2)
        pygame.draw.line(surface, color, (x, y), (x + 6, y + wing), 2)
        pygame.draw.rect(surface, (34, 30, 40, 180), (x - 1, y - 1, 2, 2))
...
def _draw_underpass_lights(surface: pygame.Surface, center: int, span: int, seed: int) -> None:
    """Restrained deck practicals — warm sodium, not cyan/magenta signage."""

    half = min(285, max(180, span // 2))
    left, right = center - half, center + half
    # Soft cool shadow under the deck only; keep alpha low so asphalt texture survives.
    pygame.draw.rect(surface, (10, 14, 20, 36), (left, 151, max(1, right - left), 102))
    for index, x in enumerate(range(left + 34, right - 20, 74)):
        # Warm practicals only — matches target still language.
        lamp = (255, 196, 128, 200) if (index + seed) % 2 else (232, 178, 110, 190)
        pygame.draw.rect(surface, (28, 30, 34, 220), (x - 5, 157, 12, 4))
        pygame.draw.rect(surface, lamp, (x - 3, 159, 7, 2))
        pygame.draw.polygon(
            surface,
            (*lamp[:3], 14),
            [(x - 3, 162), (x + 4, 162), (x + 18, 248), (x - 16, 248)],
        )


def _draw_underpass_glow(surface: pygame.Surface, center: int, span: int, seed: int) -> None:
    """Subtle living light — cool concrete wash + sparse warm ground chips only."""

    half = min(320, max(200, span // 2))
    left, right = center - half, center + half
    pulse = seed % 3
    # Cool gray-blue deck wash (not electric cyan).
    for band, alpha in ((150, 10 + pulse), (166, 7), (182, 5)):
        pygame.draw.rect(
            surface,
            (72, 86, 98, alpha),
            (left + 16, band, max(1, right - left - 32), 5),
        )
    # Quiet warm edge at portal openings — synth stop without candy neon bars.
    for edge_x in (left + 22, right - 30):
        pygame.draw.rect(surface, (255, 160, 110, 12 + pulse), (edge_x, 176, 2, 64))
    # Sparse warm asphalt catches (sunset reflection), never magenta/cyan chips.
    width = surface.get_width()
    for index in range(5):
        chip_x = left + 48 + ((seed * 19 + index * 97) % max(60, right - left - 90))
        chip_y = 320 + (index * 5 + seed) % 16
        if chip_x < -8 or chip_x > width + 8:
            continue
        color = (210, 150, 96, 36) if index % 2 else (170, 130, 100, 28)
        pygame.draw.ellipse(surface, color, (chip_x, chip_y, 8 + index % 3, 2))


def _draw_foliage_sway(
    surface: pygame.Surface,
    center: int,
    span: int,
    instances: int,
    ...
    """Sparse scrub/tree tops that lean with a slow wind phase."""

    count = max(1, min(8, instances))
    half = min(360, max(140, span // 2))
    phase = motion_tick // 5
    for index in range(count):
        base_x = center - half + 30 + (seed * 29 + index * 113) % max(90, half * 2 - 60)
        base_y = 248 + (index % 3) * 4
        sway = ((phase + index * 3 + seed) % 7) - 3
        trunk_top = base_y - (22 + (index * 5 + seed) % 14)
        # Dry San Diego scrub / small tree mass — hard pixel clusters, no soft blur.
        pygame.draw.line(
            surface,
            (48, 52, 42, 200),
            (base_x, base_y),
            (base_x + sway, trunk_top),
            2,
        )
        canopy = (
            (base_x + sway - 10, trunk_top + 4),
            (base_x + sway - 2, trunk_top - 8),
            (base_x + sway + 8, trunk_top - 2),
            (base_x + sway + 6, trunk_top + 8),
            (base_x + sway - 6, trunk_top + 10),
        )
        body = (54, 78, 52, 168) if index % 2 else (72, 92, 58, 160)
        pygame.draw.polygon(surface, body, canopy)
        tip = (base_x + sway + ((phase + index) % 3) - 1, trunk_top - 6)
        pygame.draw.rect(surface, (98, 124, 72, 140), (tip[0], tip[1], 3, 2))
...
def _draw_ambient_event(
    ...
    elif kind == "underpass_glow":
        _draw_underpass_glow(surface, center, span, seed + motion_tick // 4)
    elif kind == "foliage_sway":
        _draw_foliage_sway(
            surface,
            center,
            span,
    ...
    *,
    ...
    if enemy_kind == "cart":
        ...
        if crawl:
            ...
            pygame.draw.line(sprite, (166, 142, 105), (center - 9, 55), (center + 10, 62), 2)
            pygame.draw.line(sprite, (111, 95, 76), (center + 2, 49), (center + 17, 54), 2)
            pygame.draw.rect(sprite, (191, 159, 111), (center + 25, 35, 5, 3))
        else:
            ...
            pygame.draw.line(sprite, (163, 138, 99), (center - 8, 54), (center + 8, 64), 2)
            pygame.draw.line(sprite, (114, 95, 76), (center - 10, 61), (center + 10, 61), 2)
            pygame.draw.rect(sprite, (187, 151, 105), (center + 16, 47, 5, 3))
        # Exposed face, hair and layered cloth accents keep this from reading
        # as a single low-resolution block when it shares a frame with heroes.
        pygame.draw.rect(sprite, (38, 31, 30), (center + 19, 23, 8, 4))
        pygame.draw.rect(sprite, skin_light, (center + 24, 24, 5, 6))
        pygame.draw.rect(sprite, skin_deep, (center + 27, 27, 4, 3))
        pygame.draw.rect(sprite, (48, 43, 39), (center + 14, 18, 13, 5))
        pygame.draw.rect(sprite, (194, 161, 111), (center + 17, 19, 6, 2))
    else:
